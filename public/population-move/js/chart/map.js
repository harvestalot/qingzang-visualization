define("echarts/chart/map", ["require", "./base", "zrender/shape/Text", "zrender/shape/Path", "zrender/shape/Circle", "zrender/shape/Rectangle", "zrender/shape/Line", "zrender/shape/Polygon", "zrender/shape/Ellipse", "zrender/shape/Image", "../component/dataRange", "../component/roamController", "../layer/heatmap", "../config", "../util/ecData", "zrender/tool/util", "zrender/config", "zrender/tool/event", "../util/mapData/params", "../util/mapData/textFixed", "../util/mapData/geoCoord", "../util/projection/svg", "../util/projection/normal", "../chart"], function (e) {
	...
	return t.prototype = {
			...
			for (var t, i, n, a, o = this.component.legend, r = {}, s = {}, l = {}, h = {}, m = 0, V = e.length; V > m; m++)
				if (e[m].type == U.CHART_TYPE_MAP && (e[m] = this.reformOption(e[m]), i = e[m].mapType, s[i] = s[i] || {}, s[i][m] = !0, l[i] = l[i] || e[m].mapValuePrecision, this._scaleLimitMap[i] = this._scaleLimitMap[i] || {}, e[m].scaleLimit && p.merge(this._scaleLimitMap[i], e[m].scaleLimit, !0), this._roamMap[i] = e[m].roam || this._roamMap[i], (null == this._hoverLinkMap[i] || this._hoverLinkMap[i]) && (this._hoverLinkMap[i] = e[m].dataRangeHoverLink), this._nameMap[i] = this._nameMap[i] || {}, e[m].nameMap && p.merge(this._nameMap[i], e[m].nameMap, !0), this._activeMapType[i] = !0, e[m].textFixed && p.merge(g, e[m].textFixed, !0), e[m].geoCoord && p.merge(b, e[m].geoCoord, !0), this._selectedMode[i] = this._selectedMode[i] || e[m].selectedMode, (null == this._hoverable[i] || this._hoverable[i]) && (this._hoverable[i] = e[m].hoverable), (null == this._clickable[i] || this._clickable[i]) && (this._clickable[i] = e[m].clickable), (null == this._showLegendSymbol[i] || this._showLegendSymbol[i]) && (this._showLegendSymbol[i] = e[m].showLegendSymbol), h[i] = h[i] || e[m].mapValueCalculation, t = e[m].name, this.selectedMap[t] = o ? o.isSelected(t) : !0, this.selectedMap[t])) {
					...
					for (var d = 0, c = n.length; c > d; d++) {
						...
						for (var u in n[d])
							"value" != u ? r[i][a][u] = n[d][u] : isNaN(n[d].value) || (null == r[i][a].value && (r[i][a].value = 0), r[i][a].precision = Math.max(this.getPrecision(+n[d].value), r[i][a].precision), r[i][a].value += +n[d].value, r[i][a].valueMap[m] = +n[d].value);
		...
		_clearSelected: function () {
			for (var e in this._selected)
				this._activeMapType[this._mapTypeMap[e]] || (delete this._selected[e], delete this._mapTypeMap[e])
		},
		...
		_getProjectionData: function (e, t, i) {
			...
			var s,
			l = this._mapDataMap[e].lastTransform || {
				scale: {}
			};
			...
			if (this._specialArea[e])
				for (var U in this._specialArea[e])
					o.push(this._getSpecialProjectionData(e, t, U, this._specialArea[e][U], h));
			if ("china" == e||"china1" == e||"china2" == e) {
		...
		_getTransform: function (e, t, i) {
			...
			y = l / i / c,
			g = h / u;
			if (y > g ? (y = g * i, l = c * y) : (g = y, y = g * i, h = u * g), isNaN(o))
				switch (o = o || "center", o + "") {
				case "center":
					a = Math.floor((V - l) / 2);
					break;
		...
		_findMapTypeByPos: function (e, t) {
			...
			for (var s in this._mapDataMap)
				if (i = this._mapDataMap[s].transform, i && this._roamMap[s] && this._activeMapType[s] && (n = i.left, a = i.top, o = i.width, r = i.height, e >= n && n + o >= e && t >= a && a + r >= t))
					return s
		},
			...
			if (!(this.shapeList.length <= 0)) {
				...
				for (s in d)
					if (d[s]) {
						...
						if (V > 0) {
							if (r = 1.2, null != this._scaleLimitMap[s].max && c.baseScale >= this._scaleLimitMap[s].max)
								continue
						} else if (r = 1 / 1.2, null != this._scaleLimitMap[s].min && c.baseScale <= this._scaleLimitMap[s].min)
							continue;
						c.baseScale *= r,
						c.scale.x *= r,
						c.scale.y *= r,
		...
		__onmousemove: function (e) {
			if (this._mousedown && this._isAlive) {
				...
				for (var o = 0, r = this.shapeList.length; r > o; o++)
					this.shapeList[o]._mapType == this._curMapType && (this.shapeList[o].position[0] = a.left, this.shapeList[o].position[1] = a.top, this.zr.modShape(this.shapeList[o].id));
				this.messageCenter.dispatch(U.EVENT.MAP_ROAM, e.event, {
					type: "move"
				}, this.myChart),
				this.clearEffectShape(!0),
		...
		__onroamcontroller: function (e) {
			...
			s;
			for (s in i)
				this._mapDataMap[s] && this._activeMapType[s] && (r = this._mapDataMap[s].transform, r.hasRoam = !0, r.left -= a, r.top -= n, this._mapDataMap[s].transform = r);
			for (var l = 0, h = this.shapeList.length; h > l; l++)
				s = this.shapeList[l]._mapType, i[s] && this._activeMapType[s] && (r = this._mapDataMap[s].transform, this.shapeList[l].position[0] = r.left, this.shapeList[l].position[1] = r.top, this.zr.modShape(this.shapeList[l].id));
			this.messageCenter.dispatch(U.EVENT.MAP_ROAM, e.event, {
				type: "move"
			}, this.myChart),
			this.clearEffectShape(!0),
		...
		__ondrhoverlink: function (e) {
			for (var t, i, n = 0, a = this.shapeList.length; a > n; n++)
				t = this.shapeList[n]._mapType, this._hoverLinkMap[t] && this._activeMapType[t] && (i = d.get(this.shapeList[n], "value"), null != i && i >= e.valueMin && i <= e.valueMax && this.zr.addHoverShape(this.shapeList[n]))
		},
		onclick: function (e) {
				...
				if ("single" == this._selectedMode[a])
					for (var o in this._selected)
						if (this._selected[o] && this._mapTypeMap[o] == a) {
							for (var r = 0; n > r; r++)
								this.shapeList[r].style._name == o && this.shapeList[r]._mapType == a && (this.shapeList[r].style = this.shapeList[r]._style, this.zr.modShape(this.shapeList[r].id));
							o != i && (this._selected[o] = !1)
						}
				this._selected[i] = !this._selected[i];
				for (var r = 0; n > r; r++)
					this.shapeList[r].style._name == i && this.shapeList[r]._mapType == a && (this.shapeList[r].style = this._selected[i] ? this.shapeList[r].highlightStyle : this.shapeList[r]._style, this.zr.modShape(this.shapeList[r].id));
		...
		getRect: function (e) {
			...
			for (var i = Number.MAX_VALUE, n = Number.MIN_VALUE, a = Number.MAX_VALUE, o = Number.MIN_VALUE, r = e.x || 0, s = e.y || 0, l = e.pathArray || this.buildPathArray(e.path), h = 0; h < l.length; h++)
				for (var m = l[h].points, V = 0; V < m.length; V++)
					V % 2 === 0 ? (m[V] + r < i && (i = m[V]), m[V] + r > n && (n = m[V])) : (m[V] + s < a && (a = m[V]), m[V] + s > o && (o = m[V]));
			var U;
	...
	return t.prototype = {
		...
		_buildItem: function () {
			...
			for (var y = 0; s > y; y++)
				e = r[y], o = this.getColorByIndex(y), t = this._getItemShape(m, V, U, d, this._selectedMap[y] ? o : "#ccc"), t._idx = y, t.onmousemove = this._dispatchHoverLink, this.dataRangeOption.selectedMode && (t.clickable = !0, t.onclick = this._dataRangeSelected), this.shapeList.push(new a(t)), u && (i = {
						zlevel: this.getZlevelBase(),
						z: this.getZBase(),
						style: {
							x: m + U + 5,
		...
		_buildGradient: function () {
			...
			for (var p = e("zrender/tool/color"), c = 1 / (this.dataRangeOption.color.length - 1), u = [], y = 0, g = this.dataRangeOption.color.length; g > y; y++)
	...
	var n = {
		...
		china1: {
			getGeoJson: function (i) {
				e(["./geoJson/china1_geo"], function (e) {
					i(t(e))
				})
			}
		},
		china2: {
			getGeoJson: function (i) {
				e(["./geoJson/china2_geo"], function (e) {
					i(t(e))
				})
			}
		},
			...
			getPath: function (e, t) {
				for (var i = [[[0, 3.5], [7, 11.2], [15, 11.9], [30, 7], [42, .7], [52, .7], [56, 7.7], [59, .7], [64, .7], [64, 0], [5, 0], [0, 3.5]], [[13, 16.1], [19, 14.7], [16, 21.7], [11, 23.1], [13, 16.1]], [[12, 32.2], [14, 38.5], [15, 38.5], [13, 32.2], [12, 32.2]], [[16, 47.6], [12, 53.2], [13, 53.2], [18, 47.6], [16, 47.6]], [[6, 64.4], [8, 70], [9, 70], [8, 64.4], [6, 64.4]], [[23, 82.6], [29, 79.8], [30, 79.8], [25, 82.6], [23, 82.6]], [[37, 70.7], [43, 62.3], [44, 62.3], [39, 70.7], [37, 70.7]], [[48, 51.1], [51, 45.5], [53, 45.5], [50, 51.1], [48, 51.1]], [[51, 35], [51, 28.7], [53, 28.7], [53, 35], [51, 35]], [[52, 22.4], [55, 17.5], [56, 17.5], [53, 22.4], [52, 22.4]], [[58, 12.6], [62, 7], [63, 7], [60, 12.6], [58, 12.6]], [[0, 3.5], [0, 93.1], [64, 93.1], [64, 0], [63, 0], [63, 92.4], [1, 92.4], [1, 3.5], [0, 3.5]]], n = "", a = e[0], o = e[1], r = 0, s = i.length; s > r; r++) {
					...
					for (var l = 1, h = i[r].length; h > l; l++)
						n += "L " + ((i[r][l][0] * t + a).toFixed(2) - 0) + " " + ((i[r][l][1] * t + o).toFixed(2) - 0) + " "
				}
				return n + " Z"
			}
		},
		...
		"西宁": {
			getGeoJson: function (i) {
				e(["./geoJson/xi_ning_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"海东": {
			getGeoJson: function (i) {
				e(["./geoJson/hai_dong_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"海北": {
			getGeoJson: function (i) {
				e(["./geoJson/hai_bei_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"黄南州": {
			getGeoJson: function (i) {
				e(["./geoJson/huang_nan_zhou_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"海南州": {
			getGeoJson: function (i) {
				e(["./geoJson/hai_nan_zhou_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"果洛": {
			getGeoJson: function (i) {
				e(["./geoJson/guo_luo_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"玉树州": {
			getGeoJson: function (i) {
				e(["./geoJson/yu_shu_zhou_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"海西州": {
			getGeoJson: function (i) {
				e(["./geoJson/hai_xi_zhou_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"拉萨": {
			getGeoJson: function (i) {
				e(["./geoJson/la_sa_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"昌都": {
			getGeoJson: function (i) {
				e(["./geoJson/chang_du_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"山南": {
			getGeoJson: function (i) {
				e(["./geoJson/shan_nan_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"日喀则": {
			getGeoJson: function (i) {
				e(["./geoJson/ri_ke_ze_geo"], function (e) {
					i(t(e))
				})
			}
		},
		"那曲": {
			getGeoJson: function (i) {
				e(["./geoJson/na_qv_geo"], function (e) {
					i(t(e))
				})
			}
		},
		...
		"林芝": {
			getGeoJson: function (i) {
				e(["./geoJson/lin_zhi_geo"], function (e) {
					i(t(e))
				})
			}
		},
	...
	function i(e) {
		for (var i = e.firstChild; "svg" != i.nodeName.toLowerCase() || 1 != i.nodeType; )
			i = i.nextSibling;
	...
	var o = {
		...
		LineString: function (e) {
			for (var t, i = "", n = 0, a = e.length; a > n; n++)
				t = o.makePoint(e[n]), i = 0 === n ? "M" + t.join(",") : i + "L" + t.join(",");
			return i
		},
		Polygon: function (e) {
...
}), 			define("echarts/util/mapData/geoJson/china1_geo", [], function () {
	return {
		type: "FeatureCollection",
		features: [{
				type: "Feature",
...
}),define("echarts/util/mapData/geoJson/china2_geo", [], function () {
	return {
		type: "FeatureCollection",
		features: [{
				type: "Feature",
...
}), define("echarts/util/mapData/geoJson/xi_ning_geo", [], function () {
	return {
		type: "FeatureCollection",
		features: [{
				type: "Feature",
...
}), define("echarts/util/mapData/geoJson/hai_dong_geo", [], function () {
	return {
		type: "FeatureCollection",
		features: [{
				type: "Feature",
...
}), define("echarts/util/mapData/geoJson/hai_bei_geo", [], function () {
	return {
		type: "FeatureCollection",
		features: [{
				type: "Feature",
