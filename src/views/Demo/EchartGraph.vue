<template>
  <div class="EchartGraph">
    <div
      ref="circularRingGraph"
      class="EchartGraph-container"
    ></div>
    <div
      ref="pieGraph"
      class="EchartGraph-container"
...
export default {
  ...
  data() {
    return {
      circularRingView: null,
      pieView: null,
      pieCircularView: null,
      option: null,
      legend: ['检测', '租赁', '汽配', '建材'],
      trafficWay: [
        {
  ...
  methods: {
    initChart() {
      this.circularRingView = $echartsOptions.echarts.init(
        this.$refs.circularRingGraph
      );
      this.pieView = $echartsOptions.echarts.init(this.$refs.pieGraph);
      this.pieCircularView = $echartsOptions.echarts.init(
        this.$refs.pieCircularGraph
      );
      this.initCircularRing();
      this.setChartOption();
    },
    initCircularRing() {
      this.option = {
              ...
              normal: {
                label: {
                  ...
                  formatter: (params) => {
                    let percent = 0;
                    let total = 0;
                    for (let i = 0; i < this.trafficWay.length; i += 1) {
      ...
      this.circularRingView.setOption(this.option, true);
    },
    setChartOption() {
      const { trafficWay } = this;
        ...
        this.option.series[0].data.push(
          {
            ...
            itemStyle: {
              normal: {
                borderWidth: 5,
                shadowBlur: 20,
                borderColor: $echartsOptions.color[i],
                shadowColor: $echartsOptions.color[i],
              },
            },
          },
          {
      ...
      this.circularRingView.setOption(this.option);
    },
  },
};
</script>
